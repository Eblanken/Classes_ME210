...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(PIN_SIGNAL_IN), count_falling_edges_1, FALLING);
  frequencyTimer_2.begin(determine_frequency_2, 100000);
  attachInterrupt(digitalPinToInterrupt(PIN_SIGNAL_IN_2), count_falling_edges_2, FALLING);
}
